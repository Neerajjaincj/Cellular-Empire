...
@Component({
  selector: 'app-offline-game-board',
  templateUrl: './offline-game-board.component.html',
  styleUrls: ['./offline-game-board.component.scss']
})
export class OfflineGameBoardComponent implements OnInit {
  ...
  unblockSection(playerSection: number){
    ...
    if((playerSection == 1 && this.player2Lock == false)
  ...
  runNextGeneration() {
    if (this.generationCount >= this.generationLimit) {
      if(this.gameRound == this.gameRoundLimit){
        ...
        this.pauseGame();
        this.player1buildComplete= false;
        this.player2buildComplete= false;
        this.gameRound++;
        this.generationLimit = this.generationsPerRound[this.gameRound-1];
      }
    }
